...
// Paths are env-overridable so tests can point at a throwaway file instead of
// polluting the real append-only ledger. Read lazily (not at module load) so
// the override applies regardless of ESM import-hoisting order. Production
// leaves these unset.
const manifestPath = (): string =>
  process.env.MANIFEST_PATH ?? 'state/manifest.jsonl';
const failuresPath = (): string =>
  process.env.FAILURES_PATH ?? 'state/failures.jsonl';
...
export async function append(entry: ManifestEntry): Promise<void> {
  ...
  if (entry.status === 'failed') {
    await appendFile(failuresPath(), line, 'utf8');
  }
