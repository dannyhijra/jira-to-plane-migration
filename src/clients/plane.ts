...
export class PlaneClient {
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    return withRetry(async () => {
      ...
      if (res.status === 429) {
        ...
        throw new Error(
          `Plane 429 ${path}: rate limited, retry-after ${retryAfter}s`
        );
      }
      if (!res.ok) {
  ...
  async listStates(projectId: string): Promise<PlaneState[]> {
    return this.paginatedList<PlaneState>(
      this.projectPath(projectId, '/states/')
    );
  }
  ...
  async listLabels(projectId: string): Promise<PlaneLabel[]> {
    return this.paginatedList<PlaneLabel>(
      this.projectPath(projectId, '/labels/')
    );
  }
  ...
  async listProjectMembers(projectId: string): Promise<PlaneProjectMember[]> {
    return this.paginatedList<PlaneProjectMember>(
      this.projectPath(projectId, '/members/')
    );
  }
  ...
  /**
   * Patch an existing comment's body. Author/created_at remain the API key
   * owner (immutable) — only `comment_html` is updated. Used by the sync
   * backfill path to re-render comment bodies after an adfToMarkdown fix,
   * since normal sync never touches already-migrated comments.
   */
  async updateComment(
    projectId: string,
    workItemId: string,
    ...
    return this.request<{ id: string }>(
      this.projectPath(
        projectId,
        `/issues/${workItemId}/comments/${commentId}/`
      ),
      {
        method: 'PATCH',
        body: JSON.stringify(payload)
  ...
  async listModules(projectId: string): Promise<PlaneModule[]> {
    return this.paginatedList<PlaneModule>(
      this.projectPath(projectId, '/modules/')
    );
  }
  ...
  async createRelation(
    ...
    if (!ok2xx(res.status)) {
      throw new Error(
        `issue-relation ${res.status}: ${res.body.slice(0, 300)}`
      );
    }
  ...
  async uploadAttachment(
    ...
    if (step1.status >= 500) {
      throw new PlaneAttachmentStorageError(
        `assets/v2 step 1 ${step1.status}: ${step1.body.slice(0, 300)}`
      );
    }
    ...
    try {
      ...
      if (step2.status >= 500) {
        throw new PlaneAttachmentStorageError(
          `assets/v2 step 2 ${step2.status}: ${step2.body.slice(0, 300)}`
        );
      }
    ...
    if (!ok2xx(step3.status)) {
      throw new Error(
        `assets/v2 step 3 ${step3.status}: ${step3.body.slice(0, 300)}`
      );
    }
...
async function curl(opts: CurlOpts): Promise<{ status: number; body: string }> {
  ...
  if (opts.fileField) {
    args.push(
      '-F',
      `${opts.fileField.name}=@${opts.fileField.path};type=${opts.fileField.type}`
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        return reject(
          new Error(
            `curl exited ${code}: ${Buffer.concat(errChunks).toString().slice(0, 300)}`
          )
        );
      }
      ...
      if (idx < 0) {
        return reject(
          new Error(`curl output missing status sentinel: ${out.slice(0, 200)}`)
        );
      }
...
export class PlaneAttachmentStorageError extends Error {
  ...
  constructor(
    message: string,
    opts: { backendDown: boolean } = { backendDown: true }
  ) {
    super(message);
